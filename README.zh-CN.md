...
- **最近历史**：历史面板集中展示截图、录屏、取色和文本记录，支持分类、搜索与快速再次复制；缓存数量可配置。
...
历史面板会读取保存在 `~/Library/Application Support/capcap/History` 的最近截图、录屏、取色和文本记录。点击条目可重新复制，面板内还可分类筛选、搜索、预览、删除选中项或全部历史，并在 Finder 中打开缓存目录。
