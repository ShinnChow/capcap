...
历史面板会读取保存在 `~/Library/Application Support/capcap/History` 的最近截图、录屏、取色和文本记录。点击条目可重新复制；悬停条目可通过右下角星标收藏，有收藏项时就会出现星标分类，多选时点击任意星标可批量收藏。收藏项不会被自动清理或“删除全部历史”移除。面板内还可分类筛选、搜索、预览、删除选中项，并在 Finder 中打开缓存目录。
