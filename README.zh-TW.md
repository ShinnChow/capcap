...
- **歷史面板一鍵再用。** 最近截圖和取色集中在歷史面板，再次複製只需一次點擊；完全本機、數量可設定。
- **OCR 與本機翻譯。** Apple Vision 在本機辨識圖片文字；macOS 15+ 可啟用 Apple Translation 在裝置端翻譯，也可繼續使用自有 AI 或 DeepL 服務。
- **一鍵上傳到自己的圖床。** 可選：在設定裡設定好騰訊雲 COS、七牛雲 Kodo 或阿里雲 OSS，點一下編輯器裡的上傳按鈕，公開網路連結直接進剪貼簿。金鑰只儲存在本機。
- **AppKit 原生介面。** 沒有 Electron，也沒有遙測。小、快、像 macOS 該有的樣子。
...
- **最近歷程**：歷史面板集中顯示截圖、錄影、取色和文字記錄，支援分類、搜尋、星號收藏與快速再次複製；快取數量可設定。
...
歷史面板會讀取儲存在 `~/Library/Application Support/capcap/History` 的最近截圖、錄影、取色和文字記錄。點擊項目可重新複製；將游標移到項目上可透過右下角星號收藏，有收藏項目時就會顯示星號分類，多選時點擊任一星號可批次收藏。收藏項目不會被自動清理或「刪除全部歷史記錄」移除。面板內還可分類篩選、搜尋、預覽、刪除選取項目，並在 Finder 中開啟快取目錄。
