...
- **历史面板一键复用。** 最近截图和取色集中在历史面板，再次复制只需一次点击；完全本地、数量可配置。
- **OCR 与本地翻译。** Apple Vision 在本机识别图片文字；macOS 15+ 可启用 Apple Translation 在设备端翻译，也可继续使用自有 AI 或 DeepL 服务。
- **一键上传到自己的图床。** 可选：在设置里配置好腾讯云 COS、七牛云 Kodo 或阿里云 OSS，点一下编辑器里的上传按钮，公网链接直接进剪贴板。密钥只保存在本机。
- **AppKit 原生界面。** 没有 Electron，也没有遥测。小、快、像 macOS 该有的样子。
...
- **最近历史**：历史面板集中展示截图、录屏、取色和文本记录，支持分类、搜索、星标收藏与快速再次复制；缓存数量可配置。
...
- **外部录屏兼容**：选区遮罩与编辑器默认可被外部录屏软件捕获，无需额外设置。
- **设置与本地化**：支持简体中文、繁體中文、English、日本語、한국어、Français、Русский、Tiếng Việt UI，并提供菜单栏图标开关、开机启动、权限状态、快捷键录制和历史缓存数量。
...
历史面板会读取保存在 `~/Library/Application Support/capcap/History` 的最近截图、录屏、取色和文本记录。点击条目可重新复制；悬停条目可通过右下角星标收藏，有收藏项时就会出现星标分类，多选时点击任意星标可批量收藏。收藏项不会被自动清理、关闭缓存开关、“删除全部历史”或删除选中项移除；即使对应缓存已关闭，仍可在历史面板查看收藏项。面板内还可分类筛选、搜索、预览、删除选中项，并在 Finder 中打开缓存目录。
